...
void KurokoKinematics::calcForwardKinematics(int joint_id)
{
  ...
  if (joint_id == getLinkIndex("thigh_r_front_active"))
  {
    ...
    joint_link_tree_[getLinkIndex("knee_r_passive")]->internal_joint_angle_ =
        joint_link_tree_[getLinkIndex("thigh_r_front_active")]->internal_joint_angle_ *
        joint_link_tree_[getLinkIndex("knee_r_passive")]->joint_mimic_multiplier_;
    calcForwardKinematics(getLinkIndex("knee_r_passive"));

    // Update shin_r_front_passive
    joint_link_tree_[getLinkIndex("shin_r_front_passive")]->internal_joint_angle_ =
        joint_link_tree_[getLinkIndex("shin_r_active")]->internal_joint_angle_ *
        joint_link_tree_[getLinkIndex("shin_r_front_passive")]->joint_mimic_multiplier_;
    calcForwardKinematics(getLinkIndex("shin_r_front_passive"));

    // Update thigh_r_rear_passive_mimic
    joint_link_tree_[getLinkIndex("thigh_r_rear_passive_mimic")]->internal_joint_angle_ =
        joint_link_tree_[getLinkIndex("shin_r_active")]->internal_joint_angle_ *
        joint_link_tree_[getLinkIndex("thigh_r_rear_passive_mimic")]->joint_mimic_multiplier_;
    calcForwardKinematics(getLinkIndex("thigh_r_rear_passive_mimic"));

    // Update thigh_r_rear_passive
    joint_link_tree_[getLinkIndex("thigh_r_rear_passive")]->internal_joint_angle_ =
        joint_link_tree_[getLinkIndex("thigh_r_front_active")]->internal_joint_angle_ *
        joint_link_tree_[getLinkIndex("thigh_r_rear_passive")]->joint_mimic_multiplier_;
    calcForwardKinematics(getLinkIndex("thigh_r_rear_passive"));

    // Update thigh_r_middle_passive
    joint_link_tree_[getLinkIndex("thigh_r_middle_passive")]->internal_joint_angle_ =
        joint_link_tree_[getLinkIndex("thigh_r_front_active")]->internal_joint_angle_ *
        joint_link_tree_[getLinkIndex("thigh_r_middle_passive")]->joint_mimic_multiplier_;
  ...
  if (joint_id == getLinkIndex("shin_r_front_passive"))
  {
    ...
    joint_link_tree_[getLinkIndex("ankle_r_pitch_passive")]->internal_joint_angle_ =
        joint_link_tree_[getLinkIndex("shin_r_active")]->internal_joint_angle_ *
        joint_link_tree_[getLinkIndex("ankle_r_pitch_passive")]->joint_mimic_multiplier_;
    calcForwardKinematics(getLinkIndex("ankle_r_pitch_passive"));

    // Update shin_r_rear_passive
    joint_link_tree_[getLinkIndex("shin_r_rear_passive")]->internal_joint_angle_ =
        joint_link_tree_[getLinkIndex("shin_r_active")]->internal_joint_angle_ *
        joint_link_tree_[getLinkIndex("shin_r_rear_passive")]->joint_mimic_multiplier_;
    calcForwardKinematics(getLinkIndex("shin_r_rear_passive"));
  }

  if (joint_id == getLinkIndex("thigh_l_front_active"))
  {
    // Update knee_l_passive
    joint_link_tree_[getLinkIndex("knee_l_passive")]->internal_joint_angle_ =
        joint_link_tree_[getLinkIndex("thigh_l_front_active")]->internal_joint_angle_ *
        joint_link_tree_[getLinkIndex("knee_l_passive")]->joint_mimic_multiplier_;
    calcForwardKinematics(getLinkIndex("knee_l_passive"));

    // Update shin_l_front_passive
    joint_link_tree_[getLinkIndex("shin_l_front_passive")]->internal_joint_angle_ =
        joint_link_tree_[getLinkIndex("shin_l_active")]->internal_joint_angle_ *
        joint_link_tree_[getLinkIndex("shin_l_front_passive")]->joint_mimic_multiplier_;
    calcForwardKinematics(getLinkIndex("shin_l_front_passive"));

    // Update thigh_l_rear_passive_mimic
    joint_link_tree_[getLinkIndex("thigh_l_rear_passive_mimic")]->internal_joint_angle_ =
        joint_link_tree_[getLinkIndex("shin_l_active")]->internal_joint_angle_ *
        joint_link_tree_[getLinkIndex("thigh_l_rear_passive_mimic")]->joint_mimic_multiplier_;
    calcForwardKinematics(getLinkIndex("thigh_l_rear_passive_mimic"));

    // Update thigh_l_rear_passive
    joint_link_tree_[getLinkIndex("thigh_l_rear_passive")]->internal_joint_angle_ =
        joint_link_tree_[getLinkIndex("thigh_l_front_active")]->internal_joint_angle_ *
        joint_link_tree_[getLinkIndex("thigh_l_rear_passive")]->joint_mimic_multiplier_;
    calcForwardKinematics(getLinkIndex("thigh_l_rear_passive"));

    // Update thigh_l_middle_passive
    joint_link_tree_[getLinkIndex("thigh_l_middle_passive")]->internal_joint_angle_ =
        joint_link_tree_[getLinkIndex("thigh_l_front_active")]->internal_joint_angle_ *
        joint_link_tree_[getLinkIndex("thigh_l_middle_passive")]->joint_mimic_multiplier_;
    calcForwardKinematics(getLinkIndex("thigh_l_middle_passive"));
  }

  if (joint_id == getLinkIndex("shin_l_front_passive"))
  {
    // Update shin_l_active
    joint_link_tree_[getLinkIndex("shin_l_active")]->internal_joint_angle_ =
        joint_link_tree_[getLinkIndex("shin_l_front_passive")]->internal_joint_angle_ /
        joint_link_tree_[getLinkIndex("shin_l_front_passive")]->joint_mimic_multiplier_;
    calcForwardKinematics(getLinkIndex("shin_l_active"));

    // Update ankle_l_pitch_passive
    joint_link_tree_[getLinkIndex("ankle_l_pitch_passive")]->internal_joint_angle_ =
        joint_link_tree_[getLinkIndex("shin_l_active")]->internal_joint_angle_ *
        joint_link_tree_[getLinkIndex("ankle_l_pitch_passive")]->joint_mimic_multiplier_;
    calcForwardKinematics(getLinkIndex("ankle_l_pitch_passive"));

    // Update shin_l_rear_passive
    joint_link_tree_[getLinkIndex("shin_l_rear_passive")]->internal_joint_angle_ =
        joint_link_tree_[getLinkIndex("shin_l_active")]->internal_joint_angle_ *
        joint_link_tree_[getLinkIndex("shin_l_rear_passive")]->joint_mimic_multiplier_;
                                             ...
                                             const Eigen::MatrixXd& tar_orientation, int max_iter, double ik_err)
{
  ...
  for (int iter = 0; iter < max_iter; iter++)
  {
    ...
    Eigen::MatrixXd curr_position = joint_link_tree_[to]->internal_position_;
    Eigen::MatrixXd curr_orientation = joint_link_tree_[to]->internal_orientation_;
    ...
    for (int id = 0; id < idx.size(); id++)
    {
      ...
      joint_link_tree_[joint_num]->internal_joint_angle_ += delta_angle.coeff(id);
    }
  ...
  {
    if (joint_link_tree_[joint_num]->internal_joint_angle_ >= joint_link_tree_[joint_num]->joint_limit_upper_)
    {
      limit_success = false;
      break;
    }
    else if (joint_link_tree_[joint_num]->internal_joint_angle_ <= joint_link_tree_[joint_num]->joint_limit_lower_)
    {
      limit_success = false;
      break;
    }
                                             ...
                                             const Eigen::MatrixXd& weight)
{
  ...
  for (int iter = 0; iter < max_iter; iter++)
  {
    ...
    {
      int joint_id = idx[id];
      joint_link_tree_[joint_id]->internal_joint_angle_ += delta_angle.coeff(id);
    }
                                                        ...
                                                        double pitch, double yaw)
{
  ...
  double hip_roll_to_pitch_offset_y = joint_link_tree_[getLinkIndex("hip_r_pitch")]->joint_position_.coeff(1, 0);
  double hip_pitch_to_thigh_upper_z =
      joint_link_tree_[getLinkIndex("thigh_r_front_active")]->joint_position_.coeff(2, 0);
  double thigh_length = joint_link_tree_[getLinkIndex("knee_r_passive")]->joint_position_.coeff(2, 0);
  double shin_length = joint_link_tree_[getLinkIndex("ankle_r_pitch_passive")]->joint_position_.coeff(2, 0);
  double shin_lower_to_roll_offset_z = joint_link_tree_[getLinkIndex("ankle_r_roll")]->joint_position_.coeff(2, 0);
  ...
  double hip_pitch_to_ankle_roll_distance =
      sqrt(waist_to_ankle_roll_yz_plane_distance * waist_to_ankle_roll_yz_plane_distance -
           hip_roll_to_pitch_offset_y * hip_roll_to_pitch_offset_y);
  double hip_roll = atan2(waist_to_ankle_roll_yz_plane_distance, hip_roll_to_pitch_offset_y) +
                    atan2(waist_to_ankle_roll_z, waist_to_ankle_roll_y);
  // Ankle roll
  double ankle_roll = roll - hip_roll;

  // Calculate distances and angles for thigh pitch and shin pitch using atan2
  double effective_leg_length = sqrt(pow(x, 2) + pow(z, 2));  // effective leg length from hip to ankle
  double knee_pitch = atan2(effective_leg_length, y);         // knee pitch based on leg geometry
  double ankle_pitch = atan2(z, x);                           // ankle pitch based on position

  // Calculate the thigh pitch (hip pitch - knee pitch)
  double thigh_pitch = knee_pitch - ankle_pitch;

  // Set the output joint angles
  double ankle_yaw_joint = ankle_yaw;
  double ankle_roll_joint = ankle_roll;
  double shin_pitch_joint = ankle_pitch;
  double thigh_pitch_joint = thigh_pitch;
  double hip_pitch_joint = hip_pitch;
  double hip_roll_joint = hip_roll;
                                                       ...
                                                       double pitch, double yaw)
{
  ...
  // Define link lengths based on the robot's dimensions
  double hip_roll_to_pitch_offset_y = joint_link_tree_[getLinkIndex("hip_l_pitch")]->joint_position_.coeff(1, 0);
  double hip_pitch_to_thigh_upper_z =
      joint_link_tree_[getLinkIndex("thigh_l_front_active")]->joint_position_.coeff(2, 0);
  double thigh_length = joint_link_tree_[getLinkIndex("knee_l_passive")]->joint_position_.coeff(2, 0);
  double shin_length = joint_link_tree_[getLinkIndex("ankle_l_pitch_passive")]->joint_position_.coeff(2, 0);
  double shin_lower_to_roll_offset_z = joint_link_tree_[getLinkIndex("ankle_l_roll")]->joint_position_.coeff(2, 0);
  ...
  double waist_to_ankle_roll_yz_plane_distance =
      sqrt(waist_to_ankle_roll_z * waist_to_ankle_roll_z + waist_to_ankle_roll_y * waist_to_ankle_roll_y);
  double hip_pitch_to_ankle_roll_distance =
      sqrt(std::max(0.0, waist_to_ankle_roll_yz_plane_distance * waist_to_ankle_roll_yz_plane_distance -
                             hip_roll_to_pitch_offset_y * hip_roll_to_pitch_offset_y));
  double hip_roll = atan2(waist_to_ankle_roll_yz_plane_distance, hip_roll_to_pitch_offset_y) +
  ...
  double rolled_target_x = x;
  double rolled_target_y = y * cos(hip_roll) - z * sin(hip_roll);
  double rolled_target_z = y * sin(hip_roll) + z * cos(hip_roll);
  double rolled_thigh_upper_x = -hip_pitch_to_thigh_upper_z * sin(hip_pitch);
  double rolled_thigh_upper_y = hip_roll_to_pitch_offset_y;
  double rolled_thigh_upper_z = -hip_pitch_to_thigh_upper_z * cos(hip_pitch);
  double rolled_shin_lower_x = rolled_target_x;
  double rolled_shin_lower_y = hip_roll_to_pitch_offset_y;
  double rolled_shin_lower_z = rolled_target_z + ankle_roll_to_yaw_offset_z * cos(ankle_roll - hip_roll);
  double effective_leg_length =
      sqrt(pow(rolled_shin_lower_x - rolled_thigh_upper_x, 2) + pow(rolled_shin_lower_y - rolled_thigh_upper_y, 2) +
           pow(rolled_shin_lower_z - rolled_thigh_upper_z, 2));
  double triangle_knee_angle = acos((pow(thigh_length, 2) + pow(shin_length, 2) - pow(effective_leg_length, 2)) /
                                    (2 * thigh_length * shin_length));
  double triangle_thigh_angle = acos((pow(effective_leg_length, 2) + pow(thigh_length, 2) - pow(shin_length, 2)) /
                                     (2 * effective_leg_length * thigh_length));
  double triangle_shin_angle = acos((pow(effective_leg_length, 2) + pow(shin_length, 2) - pow(thigh_length, 2)) /
                                    (2 * effective_leg_length * shin_length));
  double triangle_knee_line_angle =
      atan2(rolled_target_z - rolled_thigh_upper_z, rolled_target_x - rolled_thigh_upper_x);
  double shin_pitch = -triangle_knee_line_angle + triangle_shin_angle;
  double thigh_pitch = -triangle_knee_line_angle - triangle_thigh_angle;

  // Set the output joint angles
  double ankle_yaw_joint = ankle_yaw;
  double ankle_roll_joint = ankle_roll;
  double shin_pitch_joint = shin_pitch;
  double thigh_pitch_joint = thigh_pitch;
  double hip_pitch_joint = hip_pitch;
  double hip_roll_joint = hip_roll;
