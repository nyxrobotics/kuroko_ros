...
void UpperBodyActionModule::initialize(const int control_cycle_msec, robotis_framework::Robot* robot)
{
  ...
  std::string path = ros::package::getPath("kuroko_upper_body_action_module") + "/data/motion_4095.bin";
                                    ...
                                    std::map<std::string, double> sensors)
{
  ...
  if (present_running_ != previous_running_)
  {
    ...
    {
      for (std::map<std::string, robotis_framework::DynamixelState*>::iterator action_result_it = action_result_.begin();
...
int UpperBodyActionModule::convertRadTow4095(double rad)
{
  return (int)((rad + M_PI) * 2048.0 / M_PI);
}

double UpperBodyActionModule::convertw4095ToRad(int w4095)
{
  return (w4095 - 2048) * M_PI / 2048.0;
}

bool UpperBodyActionModule::verifyChecksum(action_file_define::Page* page)
{
...
void UpperBodyActionModule::setChecksum(action_file_define::Page* page)
{
  unsigned char checksum = 0x00;
  unsigned char* pt = (unsigned char*)page;
...
bool UpperBodyActionModule::loadFile(std::string file_name)
{
...
bool UpperBodyActionModule::createFile(std::string file_name)
{
...
bool UpperBodyActionModule::start(int page_number)
{
...
bool UpperBodyActionModule::start(std::string page_name)
{
  int index;
  action_file_define::Page page;
...
bool UpperBodyActionModule::start(int page_number, action_file_define::Page* page)
{
  if (enable_ == false)
  {
...
bool UpperBodyActionModule::isRunning(int* playing_page_num, int* playing_step_num)
{
...
bool UpperBodyActionModule::loadPage(int page_number, action_file_define::Page* page)
{
...
bool UpperBodyActionModule::savePage(int page_number, action_file_define::Page* page)
{
  long position = (long)(sizeof(action_file_define::Page) * page_number);
...
void UpperBodyActionModule::resetPage(action_file_define::Page* page)
{
  unsigned char* pt = (unsigned char*)page;
...
void UpperBodyActionModule::enableAllJoints()
{
  for (std::map<std::string, bool>::iterator it = action_joints_enable_.begin(); it != action_joints_enable_.end(); it++)
  {
    it->second = true;
  }
}
...
void UpperBodyActionModule::actionPlayProcess(std::map<std::string, robotis_framework::Dynamixel*> dxls)
{
  ...
  else if (unit_time_count >= unit_time_num)  // If current section is completed
  {
    ...
    if (section == PRE_SECTION)
    {
      ...
      // of 7.8s occurs within the specified time unit conversion ---  angle :1024->300deg,  velocity: 256 ->720 wUnitTimeNum
