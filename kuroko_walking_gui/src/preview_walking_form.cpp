...
#include "../include/kuroko_walking_gui/preview_walking_form.h"

PreviewWalkingForm::PreviewWalkingForm(QWidget* parent)
  : QWidget(parent), p_walking_ui_(new Ui::PreviewWalkingForm), is_updating_(false)
{
  p_walking_ui_->setupUi(this);
}

PreviewWalkingForm::~PreviewWalkingForm()
{
  delete p_walking_ui_;
}

bool PreviewWalkingForm::init(walking_gui::QNodeKuroko* qnode)
{
...
void PreviewWalkingForm::on_button_send_body_offset_clicked(bool /*check*/)
{
  ...
  msg.position.x = p_walking_ui_->dSpinBox_body_offset_x->value();
  msg.position.y = p_walking_ui_->dSpinBox_body_offset_y->value();
  msg.position.z = p_walking_ui_->dSpinBox_body_offset_z->value();
...
void PreviewWalkingForm::on_button_send_foot_distance_clicked(bool /*check*/)
{
  ...
  qnode_kuroko_->sendFootDistanceMsg(msg);
}
...
void PreviewWalkingForm::on_button_p_walking_init_pose_clicked(bool /*check*/)
{
  ...
  qnode_kuroko_->parseIniPoseData(ini_pose_path);

  std_msgs::Bool msg;
  msg.data = true;

  qnode_kuroko_->sendResetBodyMsg(msg);
}
...
void PreviewWalkingForm::on_button_p_walking_balance_on_clicked(bool /*check*/)
{
  ...
  qnode_kuroko_->sendWholebodyBalanceMsg(msg);
}
...
void PreviewWalkingForm::on_button_footstep_plan_clicked(bool /*check*/)
{
  ...
  //  double roll =
  //  deg2rad<double>(p_walking_ui->dSpinBox_marker_ori_r->value()); double
  //  pitch = deg2rad<double>(p_walking_ui->dSpinBox_marker_ori_p->value());
  //  double yaw =
  //  deg2rad<double>(p_walking_ui->dSpinBox_marker_ori_y->value());

  //  Eigen::Quaterniond orientation = rpy2quaternion(roll, pitch, yaw);
  //  tf::quaternionEigenToMsg(orientation, target_pose.orientation);

  qnode_kuroko_->makeFootstepUsingPlanner(target_pose);
}

void PreviewWalkingForm::on_button_footstep_clear_clicked(bool /*check*/)
{
  qnode_kuroko_->clearFootsteps();
}

void PreviewWalkingForm::on_button_footstep_go_clicked(bool /*check*/)
{
  double step_time = p_walking_ui_->dSpinBox_p_walking_step_time->value();

  qnode_kuroko_->setWalkingFootsteps(step_time);

  bool clear_path = p_walking_ui_->checkBox_clear_path->isChecked();
  if (clear_path)
    qnode_kuroko_->clearFootsteps();

  bool clear_marker = p_walking_ui_->checkBox_clear_marker->isChecked();
  if (clear_marker)
    clearMarkerPanel();
}
...
void PreviewWalkingForm::sendPWalkingCommand(const std::string& command, bool set_start_foot)
{
  ...
  msg.step_length = p_walking_ui_->dSpinBox_p_walking_step_length->value();
  msg.side_length = p_walking_ui_->dSpinBox_p_walking_side_length->value();
  msg.step_angle = p_walking_ui_->dSpinBox_p_walking_step_angle->value() * DEG2RAD;
  msg.start_leg = set_start_foot ? p_walking_ui_->comboBox_p_walking_start_leg->currentText().toStdString() : "";

  msg.command = command;

  qnode_kuroko_->sendFootStepCommandMsg(msg);
}

bool PreviewWalkingForm::setQNode(walking_gui::QNodeKuroko* qnode)
{
  if (qnode == nullptr)
    return false;
...
void PreviewWalkingForm::makeInteractiveMarker()
{
  ...
  qnode_kuroko_->makeInteractiveMarker(current_pose);
}
...
void PreviewWalkingForm::updateInteractiveMarker()
{
  ...
  qnode_kuroko_->updateInteractiveMarker(current_pose);
}
...
void PreviewWalkingForm::clearMarkerPanel()
{
  ...
  qnode_kuroko_->clearInteractiveMarker();
}
...
{
  // position
  current.position.x = p_walking_ui_->dSpinBox_marker_pos_x->value();
  current.position.y = p_walking_ui_->dSpinBox_marker_pos_y->value();
  current.position.z = p_walking_ui_->dSpinBox_marker_pos_z->value();

  // orientation
  Eigen::Vector3d euler(p_walking_ui_->dSpinBox_marker_ori_r->value(), p_walking_ui_->dSpinBox_marker_ori_p->value(),
                        p_walking_ui_->dSpinBox_marker_ori_y->value());
...
{
  // position
  p_walking_ui_->dSpinBox_marker_pos_x->setValue(current.position.x);
  p_walking_ui_->dSpinBox_marker_pos_y->setValue(current.position.y);
  p_walking_ui_->dSpinBox_marker_pos_z->setValue(current.position.z);

  // orientation
  Eigen::Vector3d euler = rad2deg<Eigen::Vector3d>(quaternion2rpy(current.orientation));

  p_walking_ui_->dSpinBox_marker_ori_r->setValue(euler[0]);
  p_walking_ui_->dSpinBox_marker_ori_p->setValue(euler[1]);
  p_walking_ui_->dSpinBox_marker_ori_y->setValue(euler[2]);
}

void PreviewWalkingForm::getPointFromMarkerPanel(geometry_msgs::Point& current)
{
  // position
  current.x = p_walking_ui_->dSpinBox_marker_pos_x->value();
  current.y = p_walking_ui_->dSpinBox_marker_pos_y->value();
  current.z = p_walking_ui_->dSpinBox_marker_pos_z->value();
}

void PreviewWalkingForm::setPointToMarkerPanel(const geometry_msgs::Point& current)
{
  // position
  p_walking_ui_->dSpinBox_marker_pos_x->setValue(current.x);
  p_walking_ui_->dSpinBox_marker_pos_y->setValue(current.y);
  p_walking_ui_->dSpinBox_marker_pos_z->setValue(current.z);

  // orientation
  p_walking_ui_->dSpinBox_marker_ori_r->setValue(0.0);
  p_walking_ui_->dSpinBox_marker_ori_p->setValue(0.0);
  p_walking_ui_->dSpinBox_marker_ori_y->setValue(0.0);
}
