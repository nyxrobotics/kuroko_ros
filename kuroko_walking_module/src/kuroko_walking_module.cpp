...
void WalkingModule::initialize(const int control_cycle_msec, robotis_framework::Robot* robot_ptr)
{
  ...
  std::string default_initial_pose_path =
      ros::package::getPath("kuroko_motion_data") + "/config/walking_initial_pose.yaml";
  ros_node.param<std::string>("initial_pose_path", initial_pose_path_, default_initial_pose_path);
...
void WalkingModule::queueThread()
{
  ...
  ros::ServiceServer get_remaining_time_server = ros_node.advertiseService(
      "/motion_control/walking/get_remaining_time", &WalkingModule::getRemainingTimeServiceCallback, this);
...
void WalkingModule::walkingCommandCallback(const std_msgs::String::ConstPtr& msg)
{
  ...
  if (msg->data == "start")
    ...
    exportInitialPose(initial_pose_path_);
}
...
bool WalkingModule::getRemainingTimeServiceCallback(kuroko_walking_module_msgs::GetFloat::Request& req,
...
void WalkingModule::synchronizeTimeParam()
{
  ...
  {
    // 4 steps to change time parameters
    double steps_to_change = 4.0;
    double time_to_change = target_walking_param_.period_time * 0.5 * steps_to_change;
...
void WalkingModule::applyStepParam()
{
  ...
  step_length_y_ = synchronized_walking_param_.y_step;
...
void WalkingModule::startWalking()
{
  ...
  if (!is_walking_)
  {
    if (target_walking_param_.y_step > 0.0001)
      time_ = 0;
    else if (target_walking_param_.y_step < -0.0001)
      time_ = walk_period_ * 0.5;
    else if (time_ < walk_period_ * 0.5)
      time_ = 0;
    else
      time_ = walk_period_ * 0.5;
    is_walking_ = true;
...
void WalkingModule::processPhase(const double& time_unit)
{
      ...
      (time_ > r_ssp_start_time_ && time_ <= r_ssp_end_time_))
  {
    ...
    double y_step_target = target_walking_param_.y_step;
    synchronizeStepParam();
    if (y_step_target < 0 && fabs(y_step_before) < fabs(y_step_target) &&
        (time_ > l_ssp_start_time_ && time_ <= l_ssp_end_time_))
      synchronized_walking_param_.y_step = y_step_before;
    else if (y_step_target > 0 && fabs(y_step_before) < fabs(y_step_target) &&
             (time_ > r_ssp_start_time_ && time_ <= r_ssp_end_time_))
      synchronized_walking_param_.y_step = y_step_before;
    applyStepParam();
  }
                                 ...
                                 Eigen::Vector3d& xyz_out, Eigen::Vector3d& rpy_out)
{
  if (!static_cast<bool>(config_walking_param_.balance_enable))
    return;
  double euler_roll = euler_in[0];
  double euler_pitch = euler_in[1] - init_hip_pitch_offset_ + init_pitch_offset_;
  // ROS_INFO("WalkingModule::gyroFeedback - euler_roll: %f, euler_pitch: %f", euler_roll, euler_pitch);
  // Gyro Feedback
  ...
  rpy_out[2] = 0;
  // Euler Feedback
  double euler_total = fabs(euler_roll) + fabs(euler_pitch);
  if (euler_total < M_PI * 0.45)
  {
    xyz_out[0] += -config_walking_param_.balance_euler_x_gain * euler_pitch;
    xyz_out[1] += -config_walking_param_.balance_euler_y_gain * euler_roll;
    xyz_out[2] += -config_walking_param_.balance_euler_zx_gain * fabs(euler_pitch) -
                  config_walking_param_.balance_euler_zy_gain * fabs(euler_roll);
    rpy_out[0] += config_walking_param_.balance_euler_roll_gain * euler_roll;
    rpy_out[1] += -config_walking_param_.balance_euler_pitch_gain * euler_pitch;
  }
  // Limit feedback
...
void WalkingModule::exportInitialPose(std::string& path)
{
  ...
  for (int i = 0; i < 3; i++)
  {
    r_target_pose[i] = body2r_foot_pos[i];
    r_target_pose[i + 3] = body2r_foot_rpy[i];
    l_target_pose[i] = body2l_foot_pos[i];
    l_target_pose[i + 3] = body2l_foot_rpy[i];
  }
  ...
  std::vector<double> leg_joints(12);
  for (int i = 0; i < 6; i++)
  {
    leg_joints[i] = r_leg_joints[i];
    leg_joints[i + 6] = l_leg_joints[i];
  ...
  for (auto& dxl : robot_ptr_->dxls_)
  {
    ...
    if (joint_table_.find(joint_name) != joint_table_.end())
      state->goal_position_ = leg_joints[joint_table_[joint_name]];
    else
      state->goal_position_ = dxl_info->dxl_state_->goal_position_;

    result[joint_name] = state;
  }
  ...
  for (const auto& pair : result)
  {
    ...
    double position = pair.second->goal_position_;

    YAML::Node joint_node;
    joint_node["enable"] = true;
    joint_node["position"] = position;
    joints_node[joint_name] = joint_node;
  }

  root["joints"] = joints_node;
  ...
  try
  {
    ...
    fout << root;
    fout.close();
    ROS_INFO("[WalkingModule] Initial pose saved to: %s", path.c_str());
  }
  catch (const std::exception& e)
  {
    ROS_ERROR("[WalkingModule] Failed to write YAML file: %s", e.what());
  }
