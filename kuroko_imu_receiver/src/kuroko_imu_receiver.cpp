...
{
KurokoImuReceiver::KurokoImuReceiver() : control_cycle_msec_(8), DEBUG_PRINT(false), present_volt_(0.0)
{
                                ...
                                std::map<std::string, robotis_framework::Sensor*> sensors)
{
  ...
  uint16_t present_volt = sensors["kuroko_imu"]->sensor_state_->bulk_read_table_["present_voltage"];
  ...
  update_time.sec = sensors["kuroko_imu"]->sensor_state_->update_time_stamp_.sec_;
  update_time.nsec = sensors["kuroko_imu"]->sensor_state_->update_time_stamp_.nsec_;
  ...
  uint8_t button_flag = sensors["kuroko_imu"]->sensor_state_->bulk_read_table_["button"];
...
double KurokoImuReceiver::getGyroValue(int raw_value)
{
  return (double)raw_value * GYRO_FACTOR * DEGREE2RADIAN;
}

// -2.0 ~ 2.0g(-32768 ~ 32768), 1g = 9.8 m/s^2
double KurokoImuReceiver::getAccValue(int raw_value)
{
  return (double)raw_value * ACCEL_FACTOR;
}

void KurokoImuReceiver::publishIMU()
{
  // fusion imu data
  ...
  double pitch =
      atan2(-result_["acc_x"], sqrt(result_["acc_y"] * result_["acc_y"] + result_["acc_z"] * result_["acc_z"]));
  double yaw = 0.0;
...
void KurokoImuReceiver::handleButton(const std::string& button_name)
{
...
void KurokoImuReceiver::publishButtonMsg(const std::string& button_name)
{
...
void KurokoImuReceiver::handleVoltage(double present_volt)
{
  ...
  if (fabs(present_volt_ - previous_volt_) >= 0.1)
  {
    ...
    publishStatusMsg((present_volt_ < 11 ? robotis_controller_msgs::StatusMsg::STATUS_WARN :
...
void KurokoImuReceiver::publishStatusMsg(unsigned int type, std::string msg)
{
...
void KurokoImuReceiver::publishDXLPowerMsg(unsigned int value)
{
...
double KurokoImuReceiver::lowPassFilter(double alpha, double x_new, double& x_old)
{
