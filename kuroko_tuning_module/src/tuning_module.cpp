...
TuningModule::TuningModule()
  : control_cycle_msec_(0), has_goal_joints_(false), ini_pose_only_(false), get_tuning_data_(false)
{
  enable_ = false;
...
void TuningModule::initialize(const int control_cycle_msec, robotis_framework::Robot* robot)
{
  ...
  sync_write_pub_ = ros_node.advertise<robotis_controller_msgs::SyncWriteItem>("/motion_control/sync_write_item", 1);
...
void TuningModule::moveToInitPose()
{
  if (!enable_)
  {
    ...
    // wait for changing the module to initial_pose_module and getting the goal
    // position
...
void TuningModule::moveToTunePose(const std::string& pose_name)
{
  if (!enable_)
  {
    ROS_ERROR("kuroko_tuning_module is not enable!!");
    return;
  }
...
bool TuningModule::parseTunePoseData(const std::string& path, const std::string& pose_name)
{
  ...
  if (move_time.size() != target_pose_name.size() || move_time.empty())
    return false;
...
void TuningModule::queueThread()
{
  ...
  ros::Subscriber ini_pose_msg_sub =
      ros_node.subscribe("/motion_control/tuning_module/tuning_pose", 5, &TuningModule::tunePoseMsgCallback, this);

  joint_offset_data_sub_ = ros_node.subscribe("/motion_control/tuning_module/joint_offset_data", 10,
                                              &TuningModule::jointOffsetDataCallback, this);
  joint_gain_data_sub_ = ros_node.subscribe("/motion_control/tuning_module/joint_gain_data", 10,
                                            &TuningModule::jointGainDataCallback, this);
  joint_torque_enable_sub_ = ros_node.subscribe("/motion_control/tuning_module/torque_enable", 10,
                                                &TuningModule::jointTorqueOnOffCallback, this);
  command_sub_ = ros_node.subscribe("/motion_control/tuning_module/command", 5, &TuningModule::commandCallback, this);
  ...
  enable_offset_pub_ = ros_node.advertise<std_msgs::Bool>("/motion_control/enable_offset", 1);
  load_offset_client_ = ros_node.serviceClient<robotis_controller_msgs::LoadOffset>("/motion_control/load_offset");
...
{
  if (tuning_module_state_->is_generating_)
  {
    ROS_ERROR("Previous pose is generating now.");
    return;
  }

  if (tuning_module_state_->is_moving_)
  {
  ...
  for (int id = 1; id <= MAX_JOINT_ID; id++)
  {
    ...
    else
    {
      ...
      tra = robotis_framework::calcMinimumJerkTraWithViaPoints(
          tuning_module_state_->via_num_, ini_value, 0.0, 0.0, via_value, d_via_value, dd_via_value, tar_value, 0.0,
                           ...
                           std::map<std::string, double> /*sensors*/)
{
  ...
  if ((tuning_module_state_->cnt_ >= tuning_module_state_->all_time_steps_) && (tuning_module_state_->is_moving_))
  {
...
void TuningModule::setCtrlModule(const std::string& /*module*/)
{
...
void TuningModule::jointOffsetDataCallback(const op3_tuning_module_msgs::JointOffsetData::ConstPtr& msg)
{
  ...
  // goal position
  ROS_INFO_STREAM(msg->joint_name << " " << msg->goal_value << " " << msg->offset_value << " " << msg->p_gain << " "
  ...
  if (!robot_torque_enable_data_[msg->joint_name])
  {
...
void TuningModule::jointTorqueOnOffCallback(const op3_tuning_module_msgs::JointTorqueOnOffArray::ConstPtr& msg)
{
  ...
  if (!syncwrite_msg.joint_name.empty())
    ...
    op3_tuning_module_msgs::GetPresentJointOffsetData::Response& res)
{
  ...
  for (auto& map_it : robot_tuning_data_)
  {
...
bool TuningModule::loadOffsetToController(const std::string& path)
{
  ...
  if (load_offset_client_.call(load_offset_srv))
  {
    if (static_cast<bool>(load_offset_srv.response.result))
...
void TuningModule::parseDxlInit(const std::string& path)
{
  ...
  for (auto& map_it : robot_tuning_data_)
  {
    ...
    if (joint_node["position_p_gain"] != nullptr)
      joint_data->p_gain_ = joint_node["position_p_gain"].as<int>();
    else
      joint_data->p_gain_ = none_gain_;
    if (joint_node["position_i_gain"] != nullptr)
      joint_data->i_gain_ = joint_node["position_i_gain"].as<int>();
    else
      joint_data->i_gain_ = none_gain_;
    if (joint_node["position_d_gain"] != nullptr)
      joint_data->d_gain_ = joint_node["position_d_gain"].as<int>();
    else
      joint_data->d_gain_ = none_gain_;
  }
}
