...
bool WalkingModule::updateLegTargetAngles(std::vector<double>& leg_joints)
{
  ...
  // Right leg IK
  if (!kuroko_kinematics_->solveInverseKinematicsForRightLeg(right_joints, right_target_point[0], right_target_point[1],
                                                             right_target_point[2], right_target_point[3],
                                                             right_target_point[4], right_target_point[5]))
  {
  ...
  // Left leg IK
  if (!kuroko_kinematics_->solveInverseKinematicsForLeftLeg(left_joints, left_target_point[0], left_target_point[1],
                                                            left_target_point[2], left_target_point[3],
                                                            left_target_point[4], left_target_point[5]))
  {
  ...
  printf("Right  Forward Kinematics : %.4f %.4f %.4f %.4f %.4f %0.001f\n", x, y, z, roll, pitch, yaw);
  printf("Right Target Point        : %.4f %.4f %.4f %.4f %.4f %0.001f\n", right_target_point[0], right_target_point[1],
         right_target_point[2], right_target_point[3], right_target_point[4], right_target_point[5]);
  printf("Left IK : %.4f %.4f %.4f %.4f %.4f %0.001f\n", left_joints[0], left_joints[1], left_joints[2], left_joints[3],
         left_joints[4], left_joints[5]);
  kuroko_kinematics_->solveForwardKinematicsForLeftLeg(left_joints, x, y, z, roll, pitch, yaw);
  printf("Left Forward Kinematics : %.4f %.4f %.4f %.4f %.4f %0.001f\n", x, y, z, roll, pitch, yaw);
  printf("Left Target Point       : %.4f %.4f %.4f %.4f %.4f %0.001f\n", left_target_point[0], left_target_point[1],
