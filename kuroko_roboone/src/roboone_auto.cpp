  ...
  , client_(nh.serviceClient<robotis_controller_msgs::SetModule>("/motion_control/set_present_ctrl_modules"))
{
  ...
  // Camera params
  camera_height_ = 0.3;
  camera_fov_h_ = 126.0 * M_PI / 180.0;  // in radian
  camera_fov_v_ = 94.0 * M_PI / 180.0;   // in radian
  camera_start_time_ = ros::Time::now();
  camera_startup_duration_ = 0.8;
  rects_timeout_duration_ = 0.5;
  run_start_time_ = ros::Time(0);
  run_startup_duration_ = 0.1;
  ...
  walk_param_.balance_gyro_x_gain = 0.003;
  walk_param_.balance_gyro_y_gain = -0.002;
  walk_param_.balance_gyro_pitch_gain = 0.015;
  walk_param_.balance_gyro_roll_gain = -0.01;
  walk_param_.balance_gyro_zx_gain = 0.000;
  walk_param_.balance_gyro_zy_gain = 0.004;
  walk_param_.balance_euler_x_gain = 0.03;
  walk_param_.balance_euler_y_gain = -0.02;
  walk_param_.balance_euler_pitch_gain = 0.02;
  walk_param_.balance_euler_roll_gain = 0.0;
  walk_param_.balance_euler_zx_gain = -0.03;
  ...
  idle_param_.init_pose_duration = 1.0;
  ...
  action_duration_map_["crazy_kick"] = 1.88;
  action_duration_map_["crouch_down"] = 0.1;
  action_duration_map_["crouch_up"] = 0;
  action_duration_map_["getup_front"] = 3.25 + 0.5;
  action_duration_map_["getup_rear"] = 4.9 + 0.5;
  action_duration_map_["l_crazy_spin"] = 4.14;
  ...
  action_duration_map_["r_crazy_spin"] = 4.14;
  ...
  attack_data.min_distance = 0.32;
  attack_data.max_distance = 0.42;
  attack_actions_.push_back(attack_data);
...
void RobooneAuto::manageState()
{
  ...
  bool detect_stable =
      fabs(imu_rpy[1]) < stable_detect_angle_ && fabs(last_imu_.angular_velocity.y) < stable_detect_gyro_;
  bool detect_hold = fabs(imu_rpy[1]) > hold_detect_angle_ || fabs(last_imu_.angular_velocity.y) > hold_detect_gyro_;
  bool detect_squat = fabs(imu_rpy[1]) > squat_detect_angle_ || fabs(last_imu_.angular_velocity.y) > squat_detect_gyro_;
  bool detect_fall = fabs(imu_rpy[1]) > fall_detect_angle_;
  if (!stable_detected_ && detect_stable)
    stable_start_time_ = now;
  if (!hold_detected_ && detect_hold)
    hold_start_time_ = now;
  if (!squat_detected_ && detect_squat)
    squat_start_time_ = now;
  if (!fall_detected_ && detect_fall)
    fall_start_time_ = now;

  stable_detected_ = detect_stable;
  hold_detected_ = detect_hold;
  squat_detected_ = detect_squat;
  fall_detected_ = detect_fall;
  ...
  if (!action_name_.empty())
  {
    ...
    {
      double remaining_time = action_duration_ - (now - action_start_time_).toSec();
  ...
  if (current_state_ == "RUN")
  {
    if (detect_fall)
      transitionToFall();
    else if (detect_squat && !force_walk_)
      transitionToSquat();
    else if (detect_hold)
      transitionToHold();
    else
  ...
  {
    force_walk_ = true;
    squat_duration_ = (now - squat_start_time_).toSec();
    if (squat_duration_ > squat_min_duration_)
    {
      if (detect_fall)
      {
      ...
      else if (detect_stable && squat_duration_ > stable_detect_duration_)
      {
        ROS_INFO("Stable angle restored. Transitioning to HOLD.");
        transitionToHold();
      }
    }
    if (squat_duration_ > squat_max_duration_)
    {
  ...
  else if (current_state_ == "FALL")
  {
    ...
    if (detect_stable)
    {
      ROS_INFO("Stable angle restored. Transitioning to HOLD.");
      transitionToHold();
      fall_detected_time_ = now + ros::Duration(1.0);
    }
    else if (!detect_squat)
    {
      transitionToSquat();
    }
    else if ((now - fall_detected_time_).toSec() > fall_detect_duration_)
    {
...
// 初期姿勢への遷移
void RobooneAuto::transitionToIdle()
{
...
void RobooneAuto::handleRun()
{
  ...
  if (ros::Time::now() - last_rects_time_ < ros::Duration(rects_timeout_duration_))
  {
  ...
  if (ros::Time::now() - last_range_.header.stamp < ros::Duration(rects_timeout_duration_) &&
  ...
  double target_buttom_pixel = robot_detected_rect_.y + robot_detected_rect_.height;
  double target_buttom_distance = 10.0;
  if (target_buttom_pixel > last_camera_info_.height * 0.5 + 10.0)
  {
    double max_floor_project_angle = camera_fov_v_ * 0.5;
    double min_floor_project_distance = camera_height_ / tan(max_floor_project_angle);
    double target_buttom_project_angle = atan2(camera_height_ * (target_buttom_pixel - last_camera_info_.height * 0.5) /
                                                   (last_camera_info_.height * 0.5),
                                               min_floor_project_distance);
    target_buttom_distance = camera_height_ / tan(target_buttom_project_angle);
  }
  if (roboone_found)
  ...
  if (range_available)
    ...
    target_distance = target_buttom_distance;
      ...
      robot_detected_rect_.y > last_camera_info_.height * 0.8)
  {
    ...
    if (ros::Time::now() - camera_start_time_ > ros::Duration(camera_startup_duration_))
    {
                               ...
                               const jsk_recognition_msgs::RectArray::ConstPtr& rect_msg)
{
  ...
  if (label_msg->labels.empty() || class_msg->label_names.empty() || rect_msg->rects.empty())
    return;
